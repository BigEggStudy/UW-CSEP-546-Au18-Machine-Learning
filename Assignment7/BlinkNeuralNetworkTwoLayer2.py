...
class BlinkNeuralNetwork(nn.Module):
    def __init__(self, pool = 'Max', conv1_out = 16, conv1_kernel_size = 5, conv2_out = 16, conv2_kernel_size = 5, nn1_out = 20, nn2_out = 20):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(conv1_out, conv2_out, kernel_size=5, stride=1, padding=0),
            nn.ReLU()
        )
        ...
        self.fc1 = nn.Sequential(
            nn.Linear(conv2_out * 3 * 3, nn1_out, bias=True),
            nn.Sigmoid(),
            nn.Dropout(0.4)
        )
    ...
    def forward(self, x):
        ...
        # Size is (conv2_out, 6, 6)
        out = self.pool(out)
        # Size is (conv2_out, 3, 3)
        out = out.reshape(out.size(0), -1)
        # Size is (1, conv2_out * 3 * 3)
