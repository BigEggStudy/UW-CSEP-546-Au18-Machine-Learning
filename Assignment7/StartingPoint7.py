...
if __name__=='__main__':
    ...
    # for mirror, batch_size in [(False, 0), (True, 0), (True, 1)]:
    #     (xTrain, yTrain) = Featurize.Featurize(xTrainImages, yTrainRaw, mirror, batch_size, True)
    #     print(f'Training data size: {xTrain.size()}')
    ...
    # print('### Test on Test Set with Mirror Data ###')
    # (xTrain, yTrain) = Featurize.Featurize(xTrainImages, yTrainRaw, True, 0, True)
    # print(f'Training data size: {xTrain.size()}')
    ...
    # print('### Test on Test Set with Mirror and Transformed Data ###')
    # (xTrain, yTrain) = Featurize.Featurize(xTrainImages, yTrainRaw, True, 1, True)
    # print(f'Training data size: {xTrain.size()}')
    # model = BlinkNeuralNetworkTwoLayer.BlinkNeuralNetwork(pool='Max', conv1_out=6, conv1_kernel_size=5, conv2_out=16, conv2_kernel_size=3, nn1_out=120, nn2_out=84)
    # lossFunction = torch.nn.MSELoss(reduction='sum')
    # optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    ...
    # print('========== Check Dropout (Average Pooling) ==========')
    # for dropout in [0.05, 0.1, 0.15, 0.2]:
    #     print(f'Training Model with Dropout Probability as {dropout}')
    #     result = crossValidation.validate2(xTrain, yTrain, 2, 'Adam', 'Average', 1000, 0.01, 0, 6, 5, 16, 3, 120, 64, dropout=dropout)
    #     for (iteration, accuracy) in result:
    #         print(f'With iteration {iteration}')
    #         (lower, upper) = EvaluationsStub.Bound(accuracy, len(xTrainRaw))
    #         print('Accuracy from Cross Validation is %f, with lower bound %f and upper bound %f' % (accuracy, lower, upper))

    # print('========== Check Dropout (Max Pooling) ==========')
    # for dropout in [0.05, 0.1, 0.15, 0.2]:
    #     print(f'Training Model with Dropout Probability as {dropout}')
    #     result = crossValidation.validate2(xTrain, yTrain, 2, 'Adam', 'Max', 1000, 0.01, 0, 6, 5, 16, 3, 120, 64, dropout=dropout)
    #     for (iteration, accuracy) in result:
    #         print(f'With iteration {iteration}')
    #         (lower, upper) = EvaluationsStub.Bound(accuracy, len(xTrainRaw))
    #         print('Accuracy from Cross Validation is %f, with lower bound %f and upper bound %f' % (accuracy, lower, upper))
    ...
    # print('========== Final Try ==========')
    # for dropout in [0.05, 0.1, 0.15, 0.2, 0.4]:
    #     print(f'dropout={dropout}')
    #     result = crossValidation.validate2(xTrain, yTrain, 2, 'Adam', 'Max', 400, 0.01, 0, 36, 5, 96, 3, 120, 64, dropout=dropout)
    #     for (iteration, accuracy) in result:
    #         print(f'With iteration {iteration}')
    #         (lower, upper) = EvaluationsStub.Bound(accuracy, len(xTrainRaw))
    #         print('Accuracy from Cross Validation is %f, with lower bound %f and upper bound %f' % (accuracy, lower, upper))
    ...
    # print('========== Test the Best Model ==========')
    # # import BlinkNeuralNetworkTwoLayer3
    # # model = BlinkNeuralNetworkTwoLayer3.BlinkNeuralNetwork()
    # model = BlinkNeuralNetworkTwoLayer.BlinkNeuralNetwork(pool='Max', conv1_out=36, conv1_kernel_size=5, conv2_out=96, conv2_kernel_size=3, nn1_out=120, nn2_out=84, dropout=dropout)
    # lossFunction = torch.nn.MSELoss(reduction='sum')
    # optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
    ...
    print('========== Test the 2x2 Convolution Model ==========')
    import FancyBlinkNeuralNetwork
    model = FancyBlinkNeuralNetwork.BlinkNeuralNetwork()
    # model = BlinkNeuralNetworkTwoLayer.BlinkNeuralNetwork(pool='Max', conv1_out=36, conv1_kernel_size=5, conv2_out=96, conv2_kernel_size=3, nn1_out=120, nn2_out=84, dropout=dropout)
    lossFunction = torch.nn.MSELoss(reduction='sum')
    optimizer = torch.optim.Adam(model.parameters(), lr=0.01)
