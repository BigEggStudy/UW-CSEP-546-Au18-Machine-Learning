...
class DecisionTree(object):
    ...
    def __init__(self, minToSplit, random_seed, restrict_features_num):
        self.minToSplit = minToSplit
        self.random_seed = random_seed
        self.restrict_features_num = restrict_features_num
        self.restrict_features_indexes = []

    def fit(self, x, y):
        newX = self.__restrict_features(x, self.random_seed, self.restrict_features_num)
        self.root = self.__grow_tree(newX, y)

    def visualize(self, feature_names):
        self.__visualize_core(self.root, feature_names)

    def predict(self, x, threshold = 0.5):
        newX = self.__restrict_features(x, self.random_seed, self.restrict_features_num, True)
        predict_result = []
        for i in range(len(newX)):
            predict_result.append(self.__predict_core(newX[i], self.root, threshold))
    ...
    def predict_raw(self, x):
        ...
        predict_result = []
        for i in range(len(newX)):
            predict_result.append(self.__predict_core_raw(newX[i], self.root))
        return predict_result
    ...
    def __restrict_features(self, x, random_seed, restrict_features_num, use_existed = False):
        if restrict_features_num == 0:
            return x

        if not use_existed:
            random.seed(random_seed)
            for i in range(restrict_features_num):
                self.restrict_features_indexes.append(random.randint(0, len(x[0]) - 1))

        newX = []
        for features in x:
            newFeatures = []
            for index in self.restrict_features_indexes:
                newFeatures.append(features[index])
    ...
    def __predict_core_raw(self, xi, node):
        if node is None:
            print('Error! Should not show this')
            return -1
        elif node[0] is None and node[1] is None:
            return node[3] / (node[3] + node[2])
        else:
            if xi[node[2]] >= node[3]:
                return self.__predict_core_raw(xi, node[1])
            else:
                return self.__predict_core_raw(xi, node[0])
